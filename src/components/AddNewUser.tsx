...
export default class AddNewUser extends React.Component<{}, ISignupState> {
  ...
  render() {
    const { added, errors, newUserEmail } = this.state;
    const {
      email,
      name,
      surname,
    ...
    if (added) {
      return (
        <div>
          <p>You have just added a new user to the database.</p>
          <p>Please use the email address as a reference for the booking</p>
          <p>The email address is: <strong>{newUserEmail}</strong></p>
        </div>
      )
    }
          ...
          { (signup: any, { loading, error }: any) => {
            return (
              <Form style={{ textAlign: "left"}}>
                ...
                >{ loading ? "Adding user..." : "Add new user" }</Button>
              </Form>
            )
          }}
        </Mutation>
